'use client';
import { Award, Building2, Mail, Users, Star, CalendarCheck } from 'lucide-react';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';
import { StatCard } from '@/components/demo/StatCard';
import { doctor, dashboardStats, patients } from '@/lib/demo/healthcare-data';

export default function ProfilePage() {
  const specialties = ['Cardiology', 'Preventive Medicine', 'Hypertension Management', 'Echocardiography'];
    ...
    <div className="flex max-w-3xl flex-col gap-6">
      <div className="flex flex-col items-center gap-4 rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
        <Avatar className="h-24 w-24">
          <AvatarFallback className="bg-indigo/10 text-2xl font-bold text-indigo">{doctor.initials}</AvatarFallback>
        </Avatar>
        <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard icon={Users} label="Patients Treated" value={patients.length} accent="indigo" />
        <StatCard icon={CalendarCheck} label="Consultations" value={412} accent="cyan" />
        <StatCard icon={Star} label="Avg Rating" value={dashboardStats.satisfaction} suffix="%" accent="green" />
      </div>

      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <h3 className="mb-4 text-base font-bold text-foreground">Professional Details</h3>
        <div className="flex flex-col divide-y divide-border">
          {[
            { icon: Mail, label: 'Email', value: doctor.email },
            { icon: Award, label: 'License Number', value: doctor.license },
